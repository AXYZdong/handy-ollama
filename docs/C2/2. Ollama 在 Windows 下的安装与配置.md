...
| 参数                       | 标识与配置                                                                                                                              |
|--------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| OLLAMA_MODELS            | 表示模型文件的存放目录，默认目录为**当前用户目录**下的 `.ollama\models`，可表示为 `%USERPROFILE%\.ollama\models`<br />Windows 系统 **建议不要放在C盘**，可放在其他盘（如 `E:\ollama\models`） |
...
**步骤 3：重启 Ollama，并重新打开终端使环境变量生效**

环境变量设置完成后，需要重启 Ollama，使 Ollama 进程读取新的变量；同时需要重新打开命令提示符 (CMD) 或 PowerShell，以便新终端继承该变量。

* **重启 Ollama:** 如果使用 Windows 托盘应用，先退出 Ollama，再从开始菜单重新启动；如果手动运行了 `ollama serve`，先按 `Ctrl + C` 停止，再重新运行 `ollama serve`。
...
2. **验证 `OLLAMA_MODELS`:** 根据使用的终端输入对应命令。
...
- [Microsoft Learn：CMD 环境变量](https://learn.microsoft.com/windows-server/administration/windows-commands/cmd#substituting-environment-variable-values)
- [Microsoft Learn：PowerShell 环境变量](https://learn.microsoft.com/powershell/module/microsoft.powershell.core/about/about_environment_variables#use-the-variable-syntax)
